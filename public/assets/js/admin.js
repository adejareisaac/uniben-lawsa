...
async function enforceSecureSession() {
    ...
    if (!session) {
        console.warn("Unauthorized access attempt. Routing back to login node...");
        window.location.href = 'login.html';
    } else {
        console.log("Session verified successfully for:", session.user.email);
    }
}
  ...
  document.getElementById('modalContainer').innerHTML = `
    <div class="modal-overlay" id="leaderModal"><div class="modal-box">
      ...
      <form onsubmit="saveLeader(event, '${id || ''}')">
        ...
        <div class="form-group"><label class="form-label">Role *</label><input class="form-input" id="lrole" required></div>
  ...
  if (id) {
    supabaseClient.from('leadership').select('*').eq('id', id).single().then(({ data }) => {
      if (data) {
        ...
        document.getElementById('lrole').value = data.role || '';
...
registerPage('users', () => `
  <div class="page-header">
    ...
    <div class="header-actions"><button class="btn btn-gold" onclick="openUserModal()">+ Add User</button></div>
  </div>
...
`, async () => {
  ...
  tbody.innerHTML = data.map(u => `
    ...
    <td>${u.is_active ? '✅' : '❌'}</td>
    <td><button class="btn btn-outline btn-sm" onclick="openUserModal('${u.id}')">Edit</button> <button class="btn btn-danger btn-sm" onclick="deleteUser('${u.id}')">Delete</button></td></tr>
  `).join('');
});
...
window.saveUser = async function (event, id) {
  event.preventDefault();
  const payload = { full_name: document.getElementById('uname').value, email: document.getElementById('uemail').value, role: document.getElementById('urole').value, is_active: document.getElementById('uactive').checked };
  
  const { error } = id
    ? await supabaseClient.from('admin_users').update(payload).eq('id', id)
    : await supabaseClient.from('admin_users').insert(payload);

  if (error) {
    showToast("Failed to save admin seat: " + error.message, 'error');
  } else {
    closeModal('userModal');
    showToast(id ? 'Updated profile credentials!' : 'Assigned workspace role!');
    navigateTo('users');
  }
};
